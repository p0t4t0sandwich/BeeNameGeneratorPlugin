...
public class BeeNameGenerator {
    ...
    public static void start(String configPath) {
        ...
        Utils.runTaskAsync(() -> {
            // Initialize Panels
            BeeNameGeneratorPlugin.useLogger("Initializing panels...");
            initPanels();

            // Initialize servers
            BeeNameGeneratorPlugin.useLogger("Initializing servers...");
            initServers();

            // Initialize groups
            BeeNameGeneratorPlugin.useLogger("Initializing groups...");
            initGroups();
        });
            ...
            try {
                ...
                switch (panelType) {
                    ...
                    case "pterodactyl":
                        break;
                    default:
                        BeeNameGeneratorPlugin.useLogger("Panel " + panelName + " has an invalid type!");
                        break;
                }
                ...
                if (panel != null && panel.isOnline()) {
                    ...
                    BeeNameGeneratorPlugin.useLogger("Panel " + panelName + " is online!");
                } else {
                    BeeNameGeneratorPlugin.useLogger("Panel " + panelName + " is offline!");
                }
            } catch (Exception e) {
                BeeNameGeneratorPlugin.useLogger("Failed to initialize panel " + panelName + "!");
        ...
        for (String serverName: serverConfig) {
            ...
            if (panelName.equals("ampstandalone")) {
                ...
                if (server.isOnline()) {
                    ...
                    BeeNameGeneratorPlugin.useLogger("Server " + serverName + " is online!");
                } else {
                    BeeNameGeneratorPlugin.useLogger("Server " + serverName + " is offline!");
                }
                return;
            }
            ...
            if (panel == null) {
                BeeNameGeneratorPlugin.useLogger("Server " + serverName + "'s panel is offline or defined incorrectly!");
                continue;
            }
            String panelType = config.getString("panels." + panelName + ".type");
        ...
        for (String groupName: groupConfig) {
            ...
            for (String serverName: servers) {
                if (!serverExists(serverName)) {
                    BeeNameGeneratorPlugin.useLogger("Server " + serverName + " does not exist!");
                    continue;
                }
            ...
            BeeNameGeneratorPlugin.useLogger("Group " + groupName + " initialized!");
        }
    }
    ...
    public void savePanelConfig(String panelName) {
        ...
        try {
            ...
            BeeNameGeneratorPlugin.useLogger("Failed to save config!\n" + e.getMessage());
            e.printStackTrace();
        }
    }
